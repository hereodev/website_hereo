...
const OfferingPreview: React.FC<OfferingPreviewProps> = ({ art }) => {
    ...
    return (
        <div className={`max-w-64 max-h-64 border-4 border-red-500`}>
            <p>{art.SubCategory && art.SubCategory.name}</p>
        ...
        //             <span className="truncate text-wrap">{art.title.length > 12 ? art.title.slice(0, 12) + "..." : art.title}</span>
        //         </div>
        //     </div>
        // </Link>
    );
